...
class RouteTest extends TestCase
{
    ...
    private function officer(): User { return User::factory()->create(['role' => 'officer']); }
    ...
    public function test_optimize_creates_canonical_pickups_by_capacity(): void
    {
        ...
        $big = $this->vehicle('Besar', 200); $small = $this->vehicle('Kecil', 80);
        $this->report(90, .01, .02); $this->report(60, .02, .01); $this->report(50, -.01, .03);
        ...
        $this->assertSame(0, PickupTask::count());
    }

    public function test_optimize_overflows_when_capacity_exceeded(): void
    {
        config(['saycle.depot.lat' => 0.0, 'saycle.depot.lng' => 0.0]); $this->vehicle('Kecil', 40);
        $this->report(30, .01, .01); $this->report(30, .02, .02);
        $this->actingAs($this->admin())->post('/routes/optimize')->assertSessionHas('success');
    ...
    {
        config(['saycle.depot.lat' => 0.0, 'saycle.depot.lng' => 0.0]); $this->vehicle('V', 500); $this->report(10, .01, .01);
        $admin = $this->admin(); $this->actingAs($admin)->post('/routes/optimize'); $ids = Pickup::pluck('id')->all();
        $this->actingAs($admin)->post('/routes/optimize'); $this->assertSame($ids, Pickup::pluck('id')->all());
    }

    public function test_optimize_rejects_unreviewed_and_excludes_missing_coordinates(): void
    {
        config(['saycle.depot.lat' => 0.0, 'saycle.depot.lng' => 0.0]); $this->vehicle('V', 500);
        $this->report(10, .01, .01, 'submitted'); $this->report(20, null, null); $this->report(30, .02, .02);
    ...
    {
        config(['saycle.depot.lat' => 0.0, 'saycle.depot.lng' => 0.0]); $v = $this->vehicle('V', 500); $this->report(10, .01, .01);
        $admin = $this->admin(); $this->actingAs($admin)->post('/routes/optimize'); $officer = $this->officer();
        $this->actingAs($admin)->post("/routes/{$v->id}/assign", ['officer_id' => $officer->id])->assertSessionHas('success');
        $this->assertSame('assigned', Pickup::first()->status); $this->assertSame($officer->id, Pickup::first()->officer_id);
    ...
    {
        $admin = $this->admin(); $this->actingAs($admin)->post('/vehicles', ['name' => 'Pickup', 'capacity_kg' => 300])->assertSessionHas('success');
        $v = Vehicle::first(); $this->assertSame('300.00', $v->capacity_kg); $this->assertTrue($v->is_active);
        $this->actingAs($admin)->put("/vehicles/{$v->id}", ['name' => 'Box', 'capacity_kg' => 500, 'is_active' => false])->assertSessionHas('success');
        $this->assertSame('Box', $v->refresh()->name); $this->assertFalse($v->is_active);
    ...
    {
        config(['saycle.depot.lat' => 0.0, 'saycle.depot.lng' => 0.0]); $v = $this->vehicle('V', 500); $this->report(10, .01, .01); $admin = $this->admin();
