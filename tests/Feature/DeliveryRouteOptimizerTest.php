...
class DeliveryRouteOptimizerTest extends TestCase
{
    ...
    private function line(float $kg, bool $coords = true, string $reservationStatus = 'reserved'): DeliveryLine
    {
        ...
        $c = $this->contract($coords);
        $d = Delivery::create(['partner_id' => $c->partner_id, 'contract_id' => $c->id, 'service_date' => $date, 'scheduled_for' => $date, 'status' => 'planned']);
    ...
    {
        config(['saycle.depot.lat' => 0, 'saycle.depot.lng' => 0, 'saycle.osrm.base_url' => 'http://osrm.test']);

        return app(DeliveryRouteOptimizer::class)->optimize($this->serviceDate());
    }
    ...
    public function test_auth_horizon_and_depot_config(): void
    {
        ...
        config(['saycle.depot.lat' => null]);
        $this->expectException(HttpException::class);
        app(DeliveryRouteOptimizer::class)->optimize($date);
    }

    public function test_unassigned_reasons_and_capacity_split_are_persisted(): void
    {
        $this->line(5, false);
        $this->line(5, true, 'released');
        $this->line(25);
        Vehicle::create(['name' => 'V', 'capacity_kg' => 10, 'is_active' => true]);
        $result = $this->go();
        $this->assertSame(10.0, $result['assigned_kg']);
        $this->assertSame(20.0, $result['unassigned_kg']);
    ...
    public function test_phantom_planned_trips_are_cleaned_when_delivery_becomes_undispatchable(): void
    {
        $line = $this->line(5);
        Vehicle::create(['name' => 'V', 'capacity_kg' => 10, 'is_active' => true]);
        $this->go();
        $this->assertSame(1, DeliveryTrip::where('status', 'planned')->whereDate('scheduled_for', $this->serviceDate())->count());
        Delivery::findOrFail($line->delivery_id)->partner()->update(['latitude' => null, 'longitude' => null]);
        $result = $this->go();
        $this->assertSame(0, DeliveryTrip::whereIn('status', ['planned', 'assigned', 'in_progress'])->whereDate('scheduled_for', $this->serviceDate())->count());
        $this->assertSame(['no coordinates'], collect($result['unassigned'])->pluck('reason')->all());
    }

    public function test_conserves_shared_reservation_and_existing_usage_and_capacity(): void
    {
        $line = $this->line(10);
        $old = DeliveryTrip::create(['delivery_id' => $line->delivery_id, 'vehicle_id' => Vehicle::create(['name' => 'Old', 'capacity_kg' => 20, 'is_active' => true])->id, 'scheduled_for' => '2099-01-02', 'planned_kg' => 6, 'status' => 'assigned']);
        $old->lines()->create(['delivery_line_id' => $line->id, 'planned_kg' => 6]);
        Vehicle::create(['name' => 'New', 'capacity_kg' => 10, 'is_active' => true]);
        $result = $this->go();
        $this->assertSame(4.0, $result['assigned_kg']);
        $this->assertSame(10.0, (float) DeliveryTripLine::where('delivery_line_id', $line->id)->sum('planned_kg'));
    ...
    public function test_rerun_preserves_statuses_orders_and_is_idempotent(): void
    {
        $one = $this->line(3);
        $two = $this->line(3);
        $v1 = Vehicle::create(['name' => '1', 'capacity_kg' => 10, 'is_active' => true]);
        $v2 = Vehicle::create(['name' => '2', 'capacity_kg' => 10, 'is_active' => true]);
        Delivery::findOrFail($one->delivery_id)->update(['status' => 'delivered']);
        Http::fake(['http://osrm.test/*' => Http::response(['code' => 'Ok', 'routes' => [['distance' => 123, 'duration' => 45]]])]);
        $this->go();
        $ids = DeliveryTrip::where('status', 'planned')->pluck('id')->all();
        $this->assertSame('osrm', DeliveryTrip::whereIn('id', $ids)->first()->estimation_source);
        $this->go();
        $this->assertSame($ids, DeliveryTrip::where('status', 'planned')->pluck('id')->all());
        $this->assertSame([1], DeliveryTrip::where('vehicle_id', $v1->id)->where('status', 'planned')->pluck('stop_order')->all());
    }
}
