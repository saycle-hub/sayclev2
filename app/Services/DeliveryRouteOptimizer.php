...
use App\Models\{Delivery, DeliveryTrip, DeliveryTripLine, Vehicle};
use Carbon\Carbon;
...
class DeliveryRouteOptimizer
{
    public function optimize(string $date): array
    {
        ...
        return DB::transaction(function () use ($date, $depot) {
            ...
            foreach ($deliveries as $delivery) {
                ...
                $kg = array_sum($loads);
                if (!$kg) { $unassigned[]=['delivery_id'=>$delivery->id,'kg'=>0,'reason'=>'no reserved load']; continue; }
                if (!$delivery->partner || $delivery->partner->latitude === null || $delivery->partner->longitude === null) { $unassigned[]=['delivery_id'=>$delivery->id,'kg'=>$kg,'reason'=>'no coordinates']; continue; }
                foreach ($vehicles as $vehicleId => $free) {
                    if ($kg <= .00001 || $free <= .00001) continue;
                    $take = min($kg, $free); $key = $delivery->id . ':' . $vehicleId;
                    $trip = $replaceByKey->get($key);
                    if ($trip) $trip->update(['scheduled_for'=>$delivery->scheduled_for,'stop_order'=>count($orders[$vehicleId]??[])+1,'planned_kg'=>$take,'estimation_source'=>'haversine']);
                    else $trip = $delivery->trips()->create(['vehicle_id'=>$vehicleId,'scheduled_for'=>$delivery->scheduled_for,'stop_order'=>count($orders[$vehicleId]??[])+1,'planned_kg'=>$take,'status'=>'planned','estimation_source'=>'haversine']);
                    $left=$take; foreach($loads as $lineId=>&$load){$part=min($left,$load);if($part>0){$trip->lines()->create(['delivery_line_id'=>$lineId,'planned_kg'=>$part]);$load-=$part;$left-=$part;}if($left<=.00001)break;}unset($load);
                    $vehicles[$vehicleId]-=$take; $kg-=$take; $assigned+=$take; $orders[$vehicleId][]=['delivery'=>$delivery,'trip'=>$trip];
                }
                if($kg>.00001)$unassigned[]=['delivery_id'=>$delivery->id,'kg'=>round($kg,2),'reason'=>'no vehicle capacity'];
            }
            foreach ($orders as $vehicleId => $stops) $this->metrics($stops, $depot);
    ...
    {
        $lat=config('saycle.depot.lat'); $lng=config('saycle.depot.lng');
        if($lat===null || $lng===null || !is_numeric($lat) || !is_numeric($lng)) abort(422,'Delivery optimizer requires SAYCLE_DEPOT_LAT and SAYCLE_DEPOT_LNG.');
        return ['lat'=>(float)$lat,'lng'=>(float)$lng];
    }
    private function metrics(array $stops, array $depot): void
    {
        $from=$depot; foreach($stops as $item){$stop=$item['delivery'];$to=['lat'=>(float)$stop->partner->latitude,'lng'=>(float)$stop->partner->longitude];[$m,$s]=$this->leg($from,$to);$item['trip']->update(['distance_m'=>$m,'duration_s'=>$s,'estimation_source'=>$this->source]);$from=$to;}
    }
    private string $source='haversine';
    private function leg(array $a,array $b):array
    { try{$r=Http::timeout((int)config('saycle.osrm.timeout',4))->get(rtrim(config('saycle.osrm.base_url'),'/')."/route/v1/driving/{$a['lng']},{$a['lat']};{$b['lng']},{$b['lat']}",['overview'=>'false'])->json();if(($r['code']??null)==='Ok'&&isset($r['routes'][0]['distance'],$r['routes'][0]['duration'])){$this->source='osrm';return[$r['routes'][0]['distance'],(int)round($r['routes'][0]['duration'])];}}catch(\Throwable $e){}$this->source='haversine';$d=6371000*2*asin(sqrt(sin(deg2rad($b['lat']-$a['lat'])/2)**2+cos(deg2rad($a['lat']))*cos(deg2rad($b['lat']))*sin(deg2rad($b['lng']-$a['lng'])/2)**2));return[$d,(int)round($d/8.333)];}
}
