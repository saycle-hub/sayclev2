...
class PickupCheckinService
{
    public function complete(Pickup $sourcePickup, array $data): Pickup
    {
        ...
        try {
            ...
            if (isset($data['rejection_photo'])) {
                $staged[] = $data['rejection_photo']->store('pickup-evidence', $disk);
            }
            ...
            return DB::transaction(function () use ($sourcePickup, $data, $hash, $staged) {
                ...
                if ($pickup->completion_payload_hash === $hash && in_array($pickup->status, ['completed', 'supplier_rejected'], true)) {
                    return $pickup;
                }
                ...
                foreach ($lots as $lot) {
                    WarehouseMutation::firstOrCreate(['classification_lot_id' => $lot->id, 'type' => 'receipt', 'reference_type' => Pickup::class, 'reference_id' => $pickup->id], ['grade' => $lot->grade, 'intended_use' => $lot->intended_use, 'kg' => $lot->kg, 'performed_by' => auth()->id(), 'occurred_at' => now()]);
                }
                $buyPrices = Price::query()->whereIn('grade', Grade::ALL)->get()->keyBy('grade');
                foreach ($lots as $lot) {
                    $unitPrice = (float) ($buyPrices[$lot->grade]->buy_price ?? 0);
                    $payment = FinancialLine::firstOrCreate(
                    ...
                    // Cash on pickup (D5/D7): supplier is paid at weigh-in, so
                    // only lines created in this run settle immediately.
    ...
    private function validateGrades(array $data): void
    {
        ...
        foreach ($grades as $row) {
            abort_unless(in_array($row['grade'], Grade::ALL, true) && (float) $row['kg'] > 0, 422, 'Invalid grade payload.');
        }
    }
